...
import { AlertTriangle, Clock, RefreshCcw, Search, Filter, Trash2, Cpu } from "lucide-react";
import { Card } from "./ui";
...
export function Errors() {
  ...
  const filteredErrors = useMemo(() => {
    return errors.filter(err => {
      if (search && !err.message.toLowerCase().includes(search.toLowerCase())) return false;
      if (filter !== "ALL") {
        if (filter === "DATA" && !err.message.toLowerCase().includes("market data")) return false;
        if (filter === "API" && !err.message.toLowerCase().includes("api")) return false;
        if (filter === "DB" && !err.message.toLowerCase().includes("firestore")) return false;
      }
      return true;
    });
  ...
  return (
    <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold tracking-tight text-brand-text">System Errors</h2>
        <div className="flex gap-2">
          <button
            onClick={() => {
              const errorText = errors.slice(0, 5).map(e => `${new Date(e.time).toISOString()} - ${e.message}`).join("\n");
              window.dispatchEvent(new CustomEvent('ask-ai', { detail: `Analyze these errors:\n${errorText}` }));
            }}
      ...
      <Card className="flex flex-col gap-4 p-4 border border-brand-border">
        {filteredErrors.length === 0 ? (
          <div className="text-center py-8 text-brand-text-sec flex flex-col items-center gap-2">
            ...
            <p className="text-xs max-w-[200px] leading-tight">Your filters matched no errors, or the system is free of faults.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {filteredErrors.map((err, i) => {
              ...
              return (
                <div key={i} className="flex gap-3 items-start bg-brand-danger/5 border border-brand-danger/20 p-3 rounded-xl hover:border-brand-danger/40 transition-colors">
                  <div className="bg-brand-danger/20 p-2 rounded-lg text-brand-danger shrink-0 mt-0.5">
                    <AlertTriangle className="w-4 h-4" />
                  </div>
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-brand-danger mb-1 leading-snug">{err.message}</p>
