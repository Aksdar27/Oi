...
export function validateNewsEntry(candles: OHLC[], atrMultiplierThreshold: number = 2.0) {
  ...
  else if (checklist.volatilitySpike && sweptLow && checklist.fvg === "BULLISH") {
    signalType = "BUY";
  }

  return { 
    signalType: signalType ? { type: signalType, price: entryPrice, fvg } : null, 
    checklist,
    baseATR
  };
}
